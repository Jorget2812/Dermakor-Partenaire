...
// Hardcoded for emergency sync to bypass environment issues
...
const RAW_PRODUCTS = [
    ...
    { sku: 'KRX-VTX', name: 'V-Tox Higher Power', category: 'Masque Concentré', price: 85.00, stockStatus: 'IN_STOCK', description: 'Masque efecto lifting V-Shape.' },
    // --- MESOBOOSTER AMPOULE ---
    { sku: 'KRX-MBA-BRX', name: 'Meso Booster Ampoule – Boto-RX', category: 'Meso Booster Ampoule', price: 110.00, stockStatus: 'IN_STOCK', description: 'Ampoule efecto botox-like.' },
    ...
    { sku: 'KRX-ZTX', name: 'ZE-TOX', category: 'Traitement Spécialisé', price: 120.00, stockStatus: 'IN_STOCK', description: 'Traitement détoxifiant avanzado.' },
    { sku: 'KRX-VTM', name: 'VITAMEDI Solution', category: 'Traitement Spécialisé', price: 95.00, stockStatus: 'IN_STOCK', description: 'Solution vitaminada médi-esthétique.' },
    // --- SOLUTION HYDRAFACIAL ---
    { sku: 'KRX-SH3', name: 'Solutions BioCell+ "BC1/BC2/BC3" - Pack 3 x 300 ml', category: 'Solution HydraFacial', price: 150.00, stockStatus: 'LOW_STOCK', description: 'Pack completo soluciones hydrodermabrasion.' },
    ...
    // --- GAMME HOMECARE ---
    { sku: 'KRX-HP-ACNE', name: 'Holiday Set Acné Line (Édition Limitée)', category: 'Gamme HomeCare', price: 85.00, stockStatus: 'IN_STOCK', description: 'Coffret fiestas anti-acné.' },
    { sku: 'KRX-HP-GLOW', name: 'Holiday Set Glow Line (Édition Limitée)', category: 'Gamme HomeCare', price: 85.00, stockStatus: 'IN_STOCK', description: 'Coffret fiestas éclat.' },
    { sku: 'KRX-HP-PROBIO', name: 'Holiday Set Probiotic Line (Édition Limitée)', category: 'Gamme HomeCare', price: 85.00, stockStatus: 'IN_STOCK', description: 'Coffret fiestas probiotiques.' },
    { sku: 'KRX-GC-SPP', name: 'Gamme Complète Strenghten & Protect Probiotic', category: 'Gamme HomeCare', price: 180.00, stockStatus: 'LOW_STOCK', description: 'Routine completa probiotique.' },
    { sku: 'KRX-GC-CPA', name: 'Gamme Complète Clear & Prevent Anti Acné', category: 'Gamme HomeCare', price: 175.00, stockStatus: 'LOW_STOCK', description: 'Routine completa acné.' },
    { sku: 'KRX-GC-ADG', name: 'Gamme Complète All Day Glow', category: 'Gamme HomeCare', price: 190.00, stockStatus: 'OUT_OF_STOCK', description: 'Routine completa éclat.' },
    { sku: 'KRX-GC-YFL', name: 'Gamme Clinical Line Youthplex Face Lift', category: 'Gamme HomeCare', price: 250.00, stockStatus: 'OUT_OF_STOCK', description: 'Routine completa lifting.' },
    { sku: 'KRX-CSF-LM', name: 'Clinical Skin Filler Tinted Sunscreen (Light-Medium)', category: 'Gamme HomeCare', price: 58.00, stockStatus: 'IN_STOCK', description: 'Protección solar teintée claire.' },
    { sku: 'KRX-CSF-MD', name: 'Clinical Skin Filler Tinted Sunscreen (Medium-Dark)', category: 'Gamme HomeCare', price: 58.00, stockStatus: 'IN_STOCK', description: 'Protección solar teintée foncée.' },
    { sku: 'KRX-ADGS', name: 'All Day Glow Sunblocker SPF 50', category: 'Gamme HomeCare', price: 48.00, stockStatus: 'IN_STOCK', description: 'Protección solar SPF 50 éclat.' },
    ...
    { sku: 'KRX-CRS', name: 'Cica Recovery Set', category: 'Gamme HomeCare', price: 95.00, stockStatus: 'LOW_STOCK', description: 'Set recuperación Cica.' },
    ...
    // --- ACCESSOIRE ---
    { sku: 'KRX-ACC-SIL', name: 'Pinceau Silicone KRX', category: 'Accessoire', price: 15.00, stockStatus: 'OUT_OF_STOCK', description: 'Pinceau aplicación silicone.' },
...
async function syncCatalog() {
    ...
    console.log('Clearing existing data (optional but safer for clean sync)...');
    // Note: If you want to keep existing IDs, skip deletion. 
    // For this massive import, upsert on name/sku is better.

    // 1. Sync Categories
    console.log('Syncing categories to DB...');
    const catToInsert = PRODUCT_CATEGORIES.map((name, index) => ({
        name,
    ...
    const { data: catData, error: catError } = await supabase
        .from('categories')
        .upsert(catToInsert, { onConflict: 'name' })
        .select();
    ...
    const catMap = catData.reduce((acc, c) => { acc[c.name] = c.id; return acc; }, {});

    // 2. Sync Products
    console.log('Syncing products to DB...');
    const prodsToInsert = RAW_PRODUCTS.map(p => ({
        sku: p.sku,
        name: p.name,
        category: p.category,
    ...
    const chunkSize = 40;
    for (let i = 0; i < prodsToInsert.length; i += chunkSize) {
        const chunk = prodsToInsert.slice(i, i + chunkSize);
        const { error: pError } = await supabase.from('products').upsert(chunk, { onConflict: 'sku' });
        if (pError) console.error(`Error chunk ${i}:`, pError);
        else console.log(`Injected chunk ${i / chunkSize + 1}`);
    }
